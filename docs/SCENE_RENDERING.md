...
- `configs/render_shader_ball.json`: material preview ball with two tilted circular grooves
  and a rounded circular pedestal.
...
```powershell
build\Release\macrofacet_experiments.exe render --config configs\render_shader_ball.json --width 64 --height 64 --spp 4 --output outputs\shader_ball_preview
```
...
The shader ball config uses explicit `material.roughness`.
